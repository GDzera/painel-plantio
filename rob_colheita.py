...
def monitorar(no_auth=False):
    ...
    def gerar_e_subir(primeira_vez=False):
        try:
            jp, fp = ("{}", [])
            jper = "null"
            jov = "null"
            ...
            caminho_html = pasta / ARQUIVO_HTML
            with open(caminho_html, 'w', encoding='utf-8') as f:
                f.write(html)
            print(f"  ✅ {ARQUIVO_HTML} gerado localmente com sucesso!")

            # Sincronização BLINDADA com o GitHub
            print("  ⏳ Sincronizando com o GitHub...")
            import subprocess
            try:
                # 1. Adiciona o arquivo
                subprocess.run(['git', 'add', ARQUIVO_HTML], check=True, capture_output=True)
                ...
                if status.stdout.strip():
                    # 3. Commita
                    msg = f"Auto update {time.strftime('%d/%m/%Y %H:%M')}"
                    ...
                    if push_process.returncode == 0:
                        print("  ✅ Upload concluído! (O site atualizará em 1~2 minutos. Use Ctrl+F5 no navegador)")
                    else:
                        print(f"  ⚠️ Aviso do Git: {push_process.stderr}")
                else:
                    print("  ✨ O painel gerado é idêntico ao da nuvem. Nada de novo para enviar.")

            except subprocess.CalledProcessError as e:
                print(f"  ❌ [ERRO GIT] Falha na sincronização!")
                erro_real = e.stderr.strip() if e.stderr else e.stdout.strip()
                print(f"  Detalhe técnico: {erro_real}")

            if ABRIR_BROWSER and primeira_vez:
                import webbrowser
                try: 
                    webbrowser.open(f'file://{caminho_html.resolve()}')
                except: 
                    pass

        except Exception as e:
            print(f"  [ERRO] Falha no processo geral: {e}")
            traceback.print_exc()

    # Executa a primeira vez
    gerar_e_subir(primeira_vez=True)
    ...
    try:
        while True:
            ...
            if np2 != mod_p or nm2 != mod_m:
                mod_p, mod_m = np2, nm2
                print(f"\n  [{time.strftime('%H:%M:%S')}] Alteração na planilha detectada! Atualizando...")
                time.sleep(2) # Dá um tempo pro Excel terminar de salvar
                gerar_e_subir(primeira_vez=False)
                print("-" * 65)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--add', nargs=2, metavar=('USER','SENHA'), help='Adicionar usuário')
